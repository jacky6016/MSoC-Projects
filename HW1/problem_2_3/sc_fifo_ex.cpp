...
double Din_1, Dout_1, Din_2, Dout_2, Din_3, Rout_1, Rout_2, Rout_3;
...
void sc_fifo_ex::sc_fifo_ex_thread1(void) {
	for (;;) {
		Dout_1 = m_pipe[2];
		for (int i = 2; i > 0; --i)
			m_pipe[i] = m_pipe[i - 1];
...
void sc_fifo_ex::sc_fifo_ex_thread2(void) {
	for (;;) {
		Dout_2 = m_pipe[5];
		for (int i = 5; i > 3; --i)
			m_pipe[i] = m_pipe[i - 1];
...
void sc_fifo_ex::sc_fifo_ex_thread3(void) {
	for (;;) {
		for (int i = 8; i > 6; --i)
			m_pipe[i] = m_pipe[i - 1];
		m_pipe[6] = D_2_3.read();
